# API: Tree-sitter экстракция и AST-ориентированное разделение

## Обзор

Модуль `extraction` предоставляет детерминированную AST-экстракцию кода через tree-sitter. Каждый файл анализируется парсером, который выдаёт узлы и ребра графа. Результаты сохраняются в SQLite-базу из [API SQLite FTS5](api-sqlite-fts5.md).
